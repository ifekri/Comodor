...
`comodor telegram start --background` survives closing the terminal and logging
...
def _command(config: Config) -> list[str]:
    ...
    """
    return [sys.executable, "-m", "comodor", "telegram", "start"]
...
def plan(config: Config) -> Unit:
    ...
    if sys.platform.startswith("linux"):
        ...
        body = (
            ...
            "Restart=on-failure\n"
            "RestartSec=10\n"
            # Telegram long-polls; a burst of restarts means the network is
            # down or the token is wrong, and hammering the API helps neither.
    ...
    if sys.platform == "darwin":
        path = Path.home() / "Library" / "LaunchAgents" / "ai.comodor.telegram.plist"
        ...
        body = (
            ...
            '<plist version="1.0">\n'
            "<dict>\n"
            "  <key>Label</key><string>ai.comodor.telegram</string>\n"
        ...
        target = f"gui/{os.getuid()}/ai.comodor.telegram"
        return Unit(
    ...
    if os.name == "nt":
        ...
        # kept so `comodor telegram service` can show what it asked for.
