...
`comodor <channel> start` blocks. That is the right shape for watching it work
...
Neither survives a reboot. That is what `comodor <channel> service install`
...
def log_file(config: Config, channel: Channel) -> Path:
    return Path(config.paths.user) / f"{channel.name}.log"
...
def state(config: Config, channel: Channel) -> State:
    """Whether the bot is running in the background, and since when."""
    path = pid_file(config, channel)
    try:
    ...
    try:
        ...
        since = 0.0
    return State(pid=pid, since=since, log=log_file(config, channel))
...
def start(config: Config, channel: Channel) -> tuple[bool, str]:
    ...
    command = [sys.executable, "-m", "comodor", channel.name, "start"]
