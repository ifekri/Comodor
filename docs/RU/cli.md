...
| `--theme NAME` | `ember`, `midnight`, `matrix`, `mono` — для того, что печатают команды; у интерфейса свои цвета |
| `--ascii` | ASCII-рамки в том, что печатают команды |
...
запуск. Чтобы изменение закрепилось, отредактируйте файл конфигурации или
снова запустите `comodor setup` — [Конфигурация](configuration.md).
...
Экспортировать его, а не сохранять, — это решение, и `comodor setup` его уважает. См.
