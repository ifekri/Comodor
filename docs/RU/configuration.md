...
### Что записывается в ваш файл

**Только то, что вы выбрали.** Это важнее, чем звучит. Ваш файл записывает
`comodor setup`, и он следует этому правилу.
...
Поэтому записывающий помнит, откуда пришло каждое значение. Значение, которое
всё ещё держится на заимствованном слое, возвращается к тому, что говорил *ваш*
файл; значение, которое выбрали вы сами, — ваше, и оно записывается.

- модель `x`, выбранная в setup → сохраняет `x`
- сохранение в репозитории, закрепляющем `max_cost_usd: 500` → не сохраняет
  ничего подобного
- сохранение с экспортированным `ANTHROPIC_API_KEY` → ключ остаётся в вашем
  окружении
...
`chain` и обходит провайдера, который постоянно отказывает.
