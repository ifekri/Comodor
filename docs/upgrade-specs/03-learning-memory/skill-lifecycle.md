...
تأیید انسانی (تفاوت عمدی با hermes):
  ...
  (این «خودبهبود آهسته» همان حلقه‌ی hermes را می‌بندد ولی هرگز بی‌اجازه مغز نمی‌بافد)
...
- **امنیت:** اسکن هر مهارت جدید با همان چک‌های hermes (prompt injection، command خطرناک، exfil) — الگوی `safety/redact.py` + یک `threat_patterns` مینیمال؛ مهارت پروژه‌ای (`.comodor/skills`) فقط بعد از trust صریح (الگوی `workspace.py`).

## نقشه‌ی پیاده‌سازی

1. `tools/skill_manage.py` — action ها با اعتبارسنجی مسیر (symlink و path-traversal رد — الگوی path_security hermes).
