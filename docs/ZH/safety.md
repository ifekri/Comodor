...
方向键在选项之间移动，`Enter` 发送；`Esc` 拒绝。*本次会话允许*在本次会话内按类别记忆——允许写入并不意味着允许运行命令，允许 `pytest` 也不意味着允许 `rm`。

想停止被询问，在卡片上选择*本次会话允许*——它会按事情的类型分别记住——或者永久生效，在你的配置里：
...
智能体写入的每个文件都会先做检查点——先前内容，保存在项目的 `.comodor/checkpoints/` 之下，按内容寻址，并附有一份日志记录每个快照属于哪个文件。无论你是否批准了那次写入、是否开启了自动批准，都会这样做。

目前还没有恢复某个检查点的命令；这个存储的存在是为了让智能体覆盖过的内容不会丢失，而处于版本控制之下的项目在它旁边还有自己的历史。
...
**你环境中的密钥留在环境中。** 如果你导出 `ANTHROPIC_API_KEY` 而不是保存它，`comodor setup` 不会把它复制进你的配置文件。用导出代替保存是一种明确的决定，它会被尊重。
