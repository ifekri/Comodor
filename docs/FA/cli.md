...
| `--theme NAME` | `ember`، `midnight`، `matrix`، `mono` — برای آنچه فرمان‌ها چاپ می‌کنند؛ رابط رنگ‌های خودش را دارد |
| `--ascii` | حاشیه‌های ASCII در آنچه فرمان‌ها چاپ می‌کنند |
...
دارند. برای ماندگاری یک تغییر، فایل پیکربندی را ویرایش کنید یا دوباره
`comodor setup` را اجرا کنید — [پیکربندی](configuration.md).
...
یک کلید به جای ذخیره‌اش یک تصمیم است، و `comodor setup` به آن احترام می‌گذارد. به
