...
### چه چیزی به فایل شما نوشته می‌شود

`comodor setup` است که فایل شما را می‌نویسد، و از این قاعده پیروی می‌کند.
...
پس نویسنده به خاطر می‌سپارد هر مقدار از کجا آمده. مقداری که هنوز هر چه یک لایهٔ
قرضی فراهم کرده بود را نگه می‌دارد، به همان چیزی برمی‌گردد که فایلِ *خودتان*
گفته بود؛ مقداری که خودتان انتخاب کرده‌اید مال شماست و نوشته می‌شود.

- انتخاب مدل `x` در setup → ماندگار می‌شود `x`
- ذخیره در مخزنی که `max_cost_usd: 500` را ثابت کرده → هیچ چیز از این دست
  ماندگار نمی‌شود
- ذخیره با `ANTHROPIC_API_KEY` ای که export شده → کلید در محیط شما می‌ماند
...
`chain` انتخاب می‌کند و از ارائه‌دهنده‌ای که مدام شکست می‌خورد رد می‌شود.
