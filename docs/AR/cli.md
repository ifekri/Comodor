...
| `--theme NAME` | `ember`، `midnight`، `matrix`، `mono` — لما تطبعه الأوامر؛ للواجهة ألوانها الخاصة |
| `--ascii` | حدود ASCII فيما تطبعه الأوامر |
...
عدّل ملف الإعداد أو شغّل `comodor setup` مرة أخرى —
...
ويحترم `comodor setup` ذلك. راجع [الإعداد](configuration.md).
