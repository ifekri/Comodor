...
### ما الذي يُكتب إلى ملفك

**ما اخترته فقط.** وهذا أهم مما يبدو. `comodor setup` هو ما يكتب ملفك،
وهو يتبع هذه القاعدة.
...
لذلك يتذكر الكاتب من أين جاءت كل قيمة. فالقيمة التي لا تزال تحمل ما زوّدتها
طبقة مستعارة تعود إلى ما قاله ملفك *أنت*؛ والقيمة التي اخترتها بنفسك
تخصك وتُكتب.

- اختيار النموذج `x` في الإعداد → يثبّت `x`
- الحفظ في مستودع يثبّت `max_cost_usd: 500` → لا يثبّت شيئًا من هذا القبيل
- الحفظ مع `ANTHROPIC_API_KEY` مُصدَّر → يبقى المفتاح في بيئتك
...
`chain` ويتجاوز مزوّدًا يواصل الفشل.
